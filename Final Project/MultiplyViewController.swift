...
class MultiplyViewController: UIViewController {
    ...
    @IBAction func userAction(_ sender: UIButton) {
        ...
        if currentAction == String(self.multiply.answer){
            self.currentScore += 1
    ...
    func alert()->Void{
        ...
        alert.addAction(UIAlertAction(title: "quit", style: .default, handler: { (action) in
            self.performSegue(withIdentifier: "mSegue", sender: nil)
        }))
    ...
    func setUpProblem()->Void{
        self.multiply.setUpTheEquation()
        self.leftSide.text = String(self.multiply.leftSide)
        self.rightSide.text = String(self.multiply.rightSide)
        let randomPick = Int(arc4random_uniform(3))
        for i in 0..<self.userSelection.count{
            if randomPick == i{
                self.userSelection[i].setTitle(String(self.multiply.answer), for: UIControlState.normal)
            }
            else{
                var condition:Int = 0
                while (true){
                    self.answer = self.multiply.answer + Int(arc4random_uniform(9))
                    for i in 0..<self.userSelection.count{
                        if String(self.answer) == self.userSelection[i].currentTitle || String(self.multiply.answer) == String(self.answer){
                            condition = 1
                            break
                        }
                    }
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        self.multiply.age = self.model.currentPlayer.age
