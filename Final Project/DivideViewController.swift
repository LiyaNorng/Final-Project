...
class DivideViewController: UIViewController {
    ...
    @IBAction func userAction(_ sender: UIButton) {
        ...
        if currentAction == String(self.divide.answer){
            self.currentScore += 1
    ...
    func alert()->Void{
        ...
        alert.addAction(UIAlertAction(title: "quit", style: .default, handler: { (action) in
            self.performSegue(withIdentifier: "dSegue", sender: nil)
        }))
    ...
    func setUpProblem()->Void{
        self.divide.setUpTheEquation()
        self.leftSide.text = String(self.divide.leftSide)
        self.rightSide.text = String(self.divide.rightSide)
        let randomPick = Int(arc4random_uniform(3))
        for i in 0..<self.userSelection.count{
            if randomPick == i{
                self.userSelection[i].setTitle(String(self.divide.answer), for: UIControlState.normal)
            }
            else{
                var condition:Int = 0
                while (true){
                    self.answer = self.divide.answer + Double(arc4random_uniform(9))
                    for i in 0..<self.userSelection.count{
                        if String(self.answer) == self.userSelection[i].currentTitle || String(self.divide.answer) == String(self.answer){
                            condition = 1
                            break
                        }
                    }
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        self.divide.age = self.model.currentPlayer.age
