...
class AddViewController: UIViewController {
    ...
    var pictureAngle:CGFloat = 0
    var pictureTimer:Timer!
    var timer:Timer!

    @IBOutlet weak var theBoard: UIImageView!
    ...
    @IBAction func userAction(_ sender: UIButton) {
        ...
        if currentAction == String(self.add.answer){
            self.currentScore += 1
            print ("Correct")
    ...
    func setUpProblem()->Void{
        self.add.setUpTheEquation()
        self.leftSide.text = String(self.add.leftSide)
        self.rightSide.text = String(self.add.rightSide)
        let randomPick = Int(arc4random_uniform(3))
        for i in 0..<self.userSelection.count{
            if randomPick == i{
                self.userSelection[i].setTitle(String(self.add.answer), for: UIControlState.normal)
            }
            else{
                var condition:Int = 0
                while (true){
                    self.answer = self.add.answer + Int(arc4random_uniform(9))
                    for i in 0..<self.userSelection.count{
                        if String(self.answer) == self.userSelection[i].currentTitle || String(self.add.answer) == String(self.answer){
                            condition = 1
                            break
                        }
                    }
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        self.add.age = self.model.currentPlayer.age
